            ...
            if os.path.exists(broadcast_path):
                ...
                for tx in broadcast_data.get('transactions', []):
                    if 'contractAddress' in tx:
                        ...
                        logger.info(f"✅ Loaded {contract_name}: {address}")
                
                return True
            else:
                logger.warning("⚠️ Broadcast file not found, using default addresses")
                # Use default addresses from deployment
                self.contracts = {
                    ...
                    'BallotCommitReveal': '0x0165878A594ca255338adfa4d48449f69242Eb8F',
                    'CommunityMultisig': '0x8A791620dd6260079BF849Dc5567aDC3F2FdC318'
                }
                return True
