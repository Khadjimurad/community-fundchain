...
class FundChainApp {
  ...
  updateTreasuryMetrics(stats) {
    ...
    this.updateElement('total-allocated-dashboard', this.formatETH(stats.total_allocated));
  ...
  updateOverviewMetrics(stats) {
    ...
    // Add new metrics
    if (stats.projects) {
      this.updateElement('pending-projects', stats.projects.pending || 0);
      this.updateElement('completed-projects', stats.projects.completed || 0);
    }
    
    if (stats.treasury) {
      this.updateElement('total-donors', stats.treasury.unique_donors || 0);
    }
  }
  ...
  async supportProjectById(project_id) {
    try {
      const result = await this.fetchJSON(`/projects/${project_id}/support`, { method: 'POST' });
  ...
  async approveProjectById(project_id) {
    try {
      const result = await this.fetchJSON(`/projects/${project_id}/approve`, { method: 'POST' });
  ...
  async rejectProjectById(project_id) {
    try {
      const result = await this.fetchJSON(`/projects/${project_id}/reject`, { method: 'POST' });
  ...
  displayCurrentVotingRound(roundInfo) {
    ...
    // Distribution block: всегда отображаем, но блокируем кнопки до финализации
    const distBlock = document.getElementById('voting-distribution');
    if (distBlock) {
      distBlock.classList.remove('hidden');
      const isFinalized = (roundInfo.phase === 'finalized' || roundInfo.phase === 'ended');
      const btnSeq = document.querySelector('button[onclick="app.planDistribution(\'sequential\')"]');
      const btnProp = document.querySelector('button[onclick="app.planDistribution(\'proportional\')"]');
      const btnApply = document.querySelector('button[onclick="app.applyDistribution(\'sequential\')"]');
      const btnFinalize = document.querySelector('button[onclick="app.finalizeRound()"]');

      // Планировать можно и до финализации (по желанию), но применять — только после
      if (btnSeq) btnSeq.disabled = false;
      if (btnProp) btnProp.disabled = false;
      if (btnApply) btnApply.disabled = !isFinalized;
      if (btnFinalize) btnFinalize.disabled = isFinalized; // если уже финализирован — скрывать/блокировать
    }
  ...
  async loadAdminSection() {
    try {
      ...
      // distribution UI moved to Voting tab
  ...
  async planDistribution(method) {
    try {
      const params = new URLSearchParams();
      params.append('method', method);
      params.append('cap', 'target');
      const plan = await this.fetchJSON(`/admin/distribution/plan?${params.toString()}`);

      const target = document.getElementById('distribution-plan-result');
      if (target) {
        const rows = (plan.projects || []).map(p => `
          <tr>
            <td><code>${(p.project_id || '').substring(0, 10)}...</code></td>
            <td>${this.escapeHtml(p.name || '')}</td>
            <td class="text-right">${this.formatETH(p.need)} ETH</td>
            <td class="text-right"><strong>${this.formatETH(p.allocated)} ETH</strong></td>
          </tr>
        `).join('');
        target.innerHTML = `
          <div class="card">
            <div class="card-body">
              <h6 class="card-title">${i18n.t('admin.distribution_plan') || 'План распределения'}</h6>
              <div class="text-muted mb-2">${i18n.t('admin.method') || 'Метод'}: ${method}, ${i18n.t('admin.budget') || 'Бюджет'}: ${this.formatETH(plan.budget)} ETH</div>
              <div class="table-responsive">
                <table class="data-table">
                  <thead><tr>
                    <th>ID</th>
                    <th>${i18n.t('projects.details_title') || 'Проект'}</th>
                    <th>${i18n.t('needed') || 'Нужно'}</th>
                    <th>${i18n.t('allocated') || 'Распределено'}</th>
                  </tr></thead>
                  <tbody>${rows || `<tr><td colspan="4" class="text-center text-muted">${i18n.t('messages.no_data') || 'Нет данных'}</td></tr>`}</tbody>
                </table>
              </div>
              <div class="mt-2">${i18n.t('admin.total_allocated') || 'Итого распределено'}: <strong>${this.formatETH(plan.total_allocated)} ETH</strong>, ${i18n.t('admin.remaining') || 'Остаток'}: ${this.formatETH(plan.remaining_budget)} ETH</div>
            </div>
          </div>
        `;
      }
    } catch (e) {
      this.showError(i18n.t('admin_modals.messages.failed_to_generate_distribution') || 'Не удалось сформировать план распределения');
    }
  }

  async applyDistribution(method) {
    try {
      const params = new URLSearchParams();
      params.append('method', method);
      params.append('cap', 'target');
      const result = await this.fetchJSON(`/admin/distribution/apply?${params.toString()}`, { method: 'POST' });
      this.showSuccess(i18n.t('admin.applied_distribution') || 'План применён');
      // Обновим дашборд и текущую вкладку
      await this.loadSectionData(this.currentSection);
    } catch (e) {
      this.showError(i18n.t('admin_modals.messages.failed_to_apply_distribution') || 'Не удалось применить план распределения');
    }
  }

  async finalizeRound() {
    try {
      const res = await this.fetchJSON('/admin/voting/finalize-round', { method: 'POST' });
      this.showSuccess(i18n.t('voting.round_finalized') || 'Раунд финализирован');
      await this.loadVotingSection();
    } catch (e) {
      this.showError(i18n.t('voting.failed_to_finalize_round') || 'Не удалось финализировать раунд');
    }
  }
