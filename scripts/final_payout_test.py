#!/usr/bin/env python3
"""
Final Payout Test for 10 Participants with REAL Data
Финальный тест для проверки выплат, статуса проекта и казны
"""
...
class FinalPayoutTester:
    """Final test for payouts, project status, and treasury."""
    ...
    async def initialize(self):
        ...
        logger.info("🚀 Initializing final payout test with REAL data...")
    ...
    async def run_final_tests(self):
        ...
        logger.info("💰 Starting final test suite for payouts and treasury...")
                    ...
                    if project['status'] == 5:  # ready_to_payout
                        ...
                        if total_funding > 0:
                            ...
                            payout = Payout(
                                ...
                                description=f"Payout for {project['name']} project"
                            )
                            session.add(payout)
            ...
            async with self.db_manager.get_session() as session:
                ...
                for payout in self.payouts:
                    ...
                    if random.random() < 0.8:  # 80% chance of successful execution
                        # Update payout status to executed
                        await session.execute(
                            text("""
                                UPDATE payouts 
                                SET status = 'executed', 
                            ...
                            """),
                            {
                                ...
                                "execution_tx_hash": f"0x{random.randint(1000000, 9999999):08x}",
                                "payout_id": payout.id
                            }
                        )
                        ...
                        # Mark as failed
                        await session.execute(
                            text("UPDATE payouts SET status = 'failed' WHERE id = :payout_id"),
                            {"payout_id": payout.id}
                        )
    ...
    async def test_final_project_status(self):
        ...
        try:
            async with self.db_manager.get_session() as session:
                ...
                # Get final project statuses
                final_statuses = await session.execute(
                    text("""
                        SELECT p.id, p.name, p.status, 
                               COALESCE(SUM(pa.amount), 0) as total_payouts
                        FROM projects p
                        LEFT JOIN payouts pa ON p.id = pa.project_id AND pa.status = 'executed'
    ...
    async def test_treasury_final_balance(self):
        ...
        try:
            async with self.db_manager.get_session() as session:
                ...
                total_payouts = await session.execute(text("SELECT SUM(amount) FROM payouts WHERE status = 'executed'"))
                total_payouts = total_payouts.fetchone()[0] or 0
...
💰 FINAL PAYOUT TEST REPORT (WITH REAL DATA)
============================================
...
"""
        ...
        # Save report to file
        report_file = f"final_payout_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
